...
#[bitmask(u8)]
#[derive(Debug, Clone, Copy, IntoPrimitive)]
pub enum InterruptIOCtlFlags {
    EdgeCtrl = Self(0b0000_0001),
    Lvl = Self(0b0000_0010),
    Od = Self(0b0000_0100),
...
const FEATURE_SIZE: usize = 70;
const READ_WRITE_LEN: usize = 0x08;

const BMA423_X_AXIS_MASK: u8 = 0b0000_0011; // 0x03;
const BMA423_X_AXIS_SIGN_MASK: u8 = 0b0000_0100; // 0x04;
const BMA423_Y_AXIS_MASK: u8 = 0b0001_1000; // 0x18;
const BMA423_Y_AXIS_SIGN_MASK: u8 = 0b0010_0000; // 0x20;
const BMA423_Z_AXIS_MASK: u8 = 0b1100_0000; // 0xC0;
const BMA423_Z_AXIS_SIGN_MASK: u8 = 0b0000_0001; // 0x01;
...
impl<I2C: I2c> Bma423Ex<I2C> {
    ...
    pub fn remap_axes(&mut self, axes_config: AxesConfig) -> Result<(), I2C::Error> {
        ...
        let x_axis = axes_config.x_axis & BMA423_X_AXIS_MASK;
        let x_axis_inv = (axes_config.x_axis_inv << 2) & BMA423_X_AXIS_SIGN_MASK;
        let y_axis = (axes_config.y_axis << 3) & BMA423_Y_AXIS_MASK;
        let y_axis_inv = (axes_config.y_axis_inv << 5) & BMA423_Y_AXIS_SIGN_MASK;
        let z_axis = (axes_config.z_axis << 6) & BMA423_Z_AXIS_MASK;
        let z_axis_inv = axes_config.z_axis_inv & BMA423_Z_AXIS_SIGN_MASK;

        let offset_axes: usize = usize::from(FeatureOffset::AxisRemap) + 1;

        feature_config[offset_axes] |= x_axis | x_axis_inv | y_axis | y_axis_inv | z_axis;
        feature_config[offset_axes + 1] |= z_axis_inv;
    ...
    pub fn enable_wrist_tilt(&mut self) -> Result<(), I2C::Error> {
        ...
        let offset_any_motion: usize = usize::from(FeatureOffset::AnyMotion) + 1;
        feature_config[offset_any_motion] |= u8::from(0xAA);
        feature_config[offset_any_motion + 1] &= !u8::from(1 << 3);
        feature_config[offset_any_motion + 2] |= u8::from(0x05);
        feature_config[offset_any_motion + 3] |= u8::from(1 << 5 | 1 << 6 | 1 << 7);

        let offset_wrist: usize = usize::from(FeatureOffset::WristTilt) + 1;
        feature_config[offset_wrist] |= u8::from(0x01);
    ...
    pub fn map_int1_feature_interrupt(
        &mut self,
        ...
        if enable {
            data[1] |= u8::from(interrupts);
        } else {
            data[1] &= u8::from(interrupts.not());
        }

        data[0] = Reg::FeatureInterrupt1Mapping.into();
        self.write(&data)?;
    ...
    fn stream_write(
        ...
        loop {
            ...
            let mut buf: [u8; READ_WRITE_LEN + 1] = [0; READ_WRITE_LEN + 1];
            buf[0] = reg.into();
            buf[1..READ_WRITE_LEN + 1].copy_from_slice(&data[index..index + inc]);
    ...
    pub fn init(&mut self, delay: &mut impl DelayNs) -> Result<(), I2C::Error> {
        ...
        self.stream_write(Reg::FeatureConfig, &BMA423_CONFIG_FILE, delay)?;
    ...
    pub fn get_fifo_config(&mut self) -> Result<[u8; 2], I2C::Error> {
        let mut data: [u8; 2] = [0, 0];

        self.write_read(Reg::FifoConfig0, &mut data)?;
    ...
    pub fn enable_fifo(&mut self) -> Result<(), I2C::Error> {
        ...
        let data0 = data[0] & !u8::from(FifoConfig0Flags::FifoTimeEn);
        ...
        self.write(&[Reg::FifoConfig0.into(), data0])?;
    ...
    pub fn read_fifo_raw(&mut self, data: &mut [u8]) -> Result<(), I2C::Error> {
